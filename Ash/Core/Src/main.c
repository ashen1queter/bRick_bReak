...
int main(void)
{
  ...
  while (1)
  {
      // Check if USB is connected, and enter sleep mode if not
      if (hpcd_USB_FS.State != HAL_PCD_STATE_CONNECTED) {
          // Enter sleep mode if USB is not connected
    	  uint8_t num = 69;
    	  UART_Send_To_First_MCU(num);
          HAL_PWR_EnterSLEEPMode(PWR_MAINREGULATOR_ON, PWR_SLEEPENTRY_WFI);
          
      } else {
          // USB is connected, proceed with UART data handling
          UART_Receive_Data();
      }
    /* USER CODE END WHILE */
...
void UART_Receive_Data(void) {
    ...
    if (HAL_UART_Receive(&huart2, &received_data, 1, 100) == HAL_OK) {
        if (received_data == 'W') {
            Send_HID_Key(0x11);  
        }
...
void UART_Send_To_First_MCU(uint8_t data){
